...
function createModelModule() {
    ...
    _.extend(ImageModel.prototype, {
        ...
        setRating: function(rating) {
            //TODO verify that rating is in range 0-5??
            this.rating=rating;
            this.modificationDate = new Date();//update modification date!
    ...
    _.extend(ImageCollectionModel.prototype, {
        ...
        addImageModel: function(imageModel) {
            ...
            var icm = this;
            var date = new Date();
            imageModel.addListener(function(model, etime) {
                _.each(icm.listeners, function(listener_fn) {
                    listener_fn(IMAGE_META_DATA_CHANGED_EVENT, icm,imageModel, eTime);
                });
            });
            
            _.each(this.listeners, function(listener_fn) {
                listener_fn(IMAGE_ADDED_TO_COLLECTION_EVENT, icm,imageModel, date);
            });
        ...
        removeImageModel: function(imageModel) {
            //var date = new Date();
            //imageModel.removeListener(listener_fn(imageModel, imageModel.)); TODO fix this
            
            var index = this.imageModels.lastIndexOf(imageModel);
            if(index > -1) {
            ...
            var date = new Date();
            _.each(this.listeners, function(listener_fn) {
                listener_fn(IMAGE_REMOVED_FROM_COLLECTION_EVENT, icm,imageModel, date);
            });
        },
